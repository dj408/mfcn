...
def convert_to_torch_dtype(
    data_type,
    int_dtype=torch.int32, 
    float_dtype=torch.float32
) -> torch.dtype:
    """
    Convert a standard Python type to a corresponding PyTorch dtype.

    Args:
        data_type: The data type to be converted. Can be of type int, float, or str.
        int_dtype: The PyTorch dtype to use for integers. Default is torch.int32.
        float_dtype: The PyTorch dtype to use for floats. Default is torch.float32.
        str_dtype: The PyTorch dtype to use for strings. Default is None (PyTorch does not support string dtype).

    Returns:
        A PyTorch dtype corresponding to the input type.
    """
    lookup = {
        int: int_dtype,
        float: float_dtype,
    ...
    if data_type in lookup:
        ...
        raise TypeError(
            f"Unsupported type {type(data_type)}."
            f" Only int, float, and str are supported."
        )
...
def multi_strat_multi_fold_idx_sample(
    ...
    for i in range(n_samples):
        ...
        # Check if any element in tup_key is a tensor and convert it to int or float if necessary
        tup_key = tuple(
            element.item() \
            if isinstance(element, torch.Tensor) and element.numel() == 1 else element \
            for element in tup_key
        )
